"""
Recompute KARI TAS metrics for 50salads runs and refresh CSV summaries.

It picks the most recent prediction pickle per `n_train_traces` from
`kari/results/kari_experiments/50salads`, computes TAS metrics, writes
`50salads_kari_ntraces_results.csv`, and updates the KARI columns inside
`50salads_combined_results_with_kari.csv`.
"""
...
# Allow importing compute_kari_tas in this folder
HERE = Path(__file__).resolve().parent
REPO_ROOT = HERE.parent
RESULTS_DIR = REPO_ROOT / "results" / "50salads"
KARI_EXP_DIR = HERE / "results" / "kari_experiments" / "50salads"

# Local import after paths are set
from compute_kari_tas import compute_kari_tas


def _latest_pred_file(n_traces: int) -> Path:
    """Return the latest KARI prediction pickle for the given n_traces."""
    pattern = f"results_50salads_n{n_traces}_seed101_*.pkl"
    ...
    def _date_key(p: Path) -> int:
        # filenames end with _YYYYMMDD.pkl
...
def compute_all_metrics() -> List[Dict]:
    rows: List[Dict] = []
    for n in range(10, 0, -1):  # run from max traces down to min
...
def write_kari_only(rows: List[Dict]) -> None:
    out_path = RESULTS_DIR / "50salads_kari_ntraces_results.csv"
...
def update_combined(rows: List[Dict]) -> None:
    combined_path = RESULTS_DIR / "50salads_combined_results_with_kari.csv"
    if not combined_path.exists():
        raise FileNotFoundError(f"Combined results CSV not found: {combined_path}")

    combined = pd.read_csv(combined_path)
    for row in rows:
        mask = (combined["dataset_name"] == "50salads") & (
            combined["n_train_traces"] == row["n_train_traces"]
        )
        if not mask.any():
