...
import numpy as np
# import grammarutils as grammarutils  # Functions moved to utils.py
import nltk.grammar
...
class BreadthFirstEarley(object):
    ...
    def parse(self, classifier_output, str_len=20, prune=20):
        ...
        # Clear previously parsed sequences to avoid stale state across runs
        # that can reference keys not present in the refreshed caches.
        self.parsed_str = []
        count = 0
        while self._queue:
            count += 1
    ...
    def compute_labels(self):
        ...
        token_strings = self._best_l.split(' ')
        tokens = []
        for token in token_strings:
            token_clean = token.strip("'\"")
            if self._mapping:
                if token_clean in self._mapping:
                    tokens.append(int(self._mapping[token_clean]))
                else:
                    raise ValueError(f"Cannot map token '{token_clean}' using provided mapping")
            else:
                tokens.append(int(token_clean))
    ...
    def compute_prob(self, prefix, tmp_grammar_prob):
        ...
        if l not in self._cached_log_prob:
            if self._mapping:
                # Try to get the action index from mapping, or use the numeric value directly
                last_action = prefix[-1]
                if last_action in self._mapping:
                    k = int(self._mapping[last_action])
                else:
                    # Assume it's already a numeric string
                    try:
                        k = int(last_action)
                    except ValueError:
                        raise ValueError(f"Cannot map action '{last_action}' to index")
            else:
                k = int(prefix[-1])
            l_minus = ' '.join(prefix[:-1])
