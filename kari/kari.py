    ...
    for i, key in enumerate(key_ordered_list):
        if len(key) > 1:
            ...
            for k_order in key_order_in_seq:
                # Keep only elements belonging to this key set and normalize to relative indices
                filtered = [k for k in k_order if k in key_number]
                key_order_in_seq_.append(tuple([key_number.index(k) for k in filtered]))
            ...
            # If no permutations match, fall back to uniform distribution to avoid crashes
            if len(num_perms) == 0 or sum(num_perms) == 0:
                num_perms = [1] * len(perms) if len(perms) > 0 else [1]
            total = sum(num_perms)
            key_prob = [num_perm / total for num_perm in num_perms]
            ...
            for seq, order in zip(seqs, key_order_in_seq_):
                # If this sequence does not contain all key actions, skip it
                if not all(k in seq for k in key):
                    continue
                try:
                    option = perms.index(order)
                except ValueError:
                    # Order not found; default to first permutation
                    option = 0
                key_indices = [lindex(seq, k) for k in key]
                key_indices.sort()
                if i != len(key_ordered_list) - 1:
                    # Find the next key occurrence after the current block; skip if missing
                    remaining_seq = seq[key_indices[-1]:]
                    next_candidates = [next_key for next_key in key_ordered_list[i + 1] if next_key in remaining_seq]
                    if len(next_candidates) == 0:
                        continue
                    next_key_indexes = min([lindex(remaining_seq, next_key) for next_key in next_candidates]) + key_indices[-1]
...
def get_grammar(seqs, args, with_seq=True):
    ...
    if with_seq:
        ...
        if len(key_actions) == 0:
            # No common key actions across sequences; fall back to empty K
            key_rule = [SingleRule('K', [''], [1.0])]
            key_seqs = ['']
        else:
...
def main():
    ...
    if len(sys.argv) > 1 and ('--help' in sys.argv or '-h' in sys.argv):
        print("KARI - Key-Action-based Recursive Induction for Temporal Action Segmentation")
        print("\nUsage: python kari.py [OPTIONS]")
        print("\nOptions:")
        print("  --result_dir DIR     Results directory to save induced grammars (default: induced_grammars)")
        print("  --source_dir DIR     Source directory of action sequences (default: source_breakfast)")
        print("  --dataset NAME       Dataset to use: breakfast or 50salads (default: breakfast)")
        print("  --seq                Extract sequences simultaneously")
        print("  --topK               Maintain Top-K number of key actions")
        print("  --K INT              Hyperparameter for K (default: 3)")
        print("  --merge              Merge all activity grammars for Breakfast")
        print("  --help, -h           Show this help message")
        print("\nExamples:")
        print("  python kari.py --dataset breakfast --source_dir source_breakfast")
        print("  python kari.py --dataset 50salads --source_dir source_50salads --topK --K 3")
        return
    ...
    if args.dataset == '50salads':
        # Use single split.txt file for 50salads
        splits = ['']  # Empty string to use split.txt directly
        source_file = os.path.join(args.source_dir, 'split.txt')
        if not os.path.exists(source_file):
            raise FileNotFoundError(f"split.txt not found at {source_file}")
    else:
        splits = [1, 2, 3, 4]

    for split in splits:
        if args.dataset == '50salads':
            # For 50salads, use the single split.txt file
            source_file = os.path.join(args.source_dir, 'split.txt')
            save_dir = os.path.join('results', args.result_dir, args.dataset, 'split')
        else:
            # For breakfast, use individual split files
            source_file = os.path.join(args.source_dir, 'split'+str(split)+'.txt')
        ...
        for activity in activities:
            ...
            if args.merge:
                ...
                if args.dataset == '50salads':
                    w_path = os.path.join(save_dir, '50salads.pcfg')
                else:
