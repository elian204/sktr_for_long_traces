#!/usr/bin/env python3
"""Leakage-safe modeling, repair rules, and TAS metrics for repair-head v2."""
...
def _fit_isotonic_maps(
    ...
    for label in range(N_CLASSES):
        ...
        calibrator.fit(
            probabilities[:, label], (targets == label).astype(float), sample_weight=weights
        )
...
def apply_isotonic(
    probabilities: np.ndarray, calibrators: Sequence[IsotonicRegression]
) -> np.ndarray:
    calibrated = np.column_stack(
        [calibrator.predict(probabilities[:, label]) for label, calibrator in enumerate(calibrators)]
    )
    sums = calibrated.sum(axis=1, keepdims=True)
    zero = sums[:, 0] <= 0
    calibrated[~zero] /= sums[~zero]
    calibrated[zero] = probabilities[zero]
    return calibrated


def fit_isotonic_probabilities(
    features: np.ndarray,
    records: pd.DataFrame,
    train_positions: np.ndarray,
    eval_positions: np.ndarray,
    calibration_group_column: str = "inner_fold",
) -> tuple[np.ndarray, Dict[str, Any]]:
    train = records.iloc[train_positions]
    groups = sorted(train[calibration_group_column].astype(int).unique())
    if len(groups) < 2:
        raise ValueError(f"Isotonic cross-calibration requires at least two groups, got {groups}")
    cross = np.zeros((len(train_positions), N_CLASSES), dtype=np.float64)
    for group in groups:
        fit_local = np.flatnonzero(train[calibration_group_column].astype(int).to_numpy() != group)
        predict_local = np.flatnonzero(train[calibration_group_column].astype(int).to_numpy() == group)
        fit_pos = train_positions[fit_local]
        predict_pos = train_positions[predict_local]
        probabilities, _ = fit_plain_probabilities(
            features[fit_pos],
            records.iloc[fit_pos]["correct_label"].to_numpy(dtype=int),
            records.iloc[fit_pos]["length"].to_numpy(dtype=float),
            features[predict_pos],
        )
        cross[predict_local] = probabilities
    targets = train["correct_label"].to_numpy(dtype=int)
    weights = train["length"].to_numpy(dtype=float)
    calibrators = _fit_isotonic_maps(cross, targets, weights)
    plain_eval, artifacts = fit_plain_probabilities(
        features[train_positions], targets, weights, features[eval_positions]
    )
    calibrated = apply_isotonic(plain_eval, calibrators)
    artifacts["isotonic_calibrators"] = calibrators
    artifacts["calibration_groups"] = groups
    return calibrated, artifacts
...
def fit_ensemble_probabilities(
    x_train: np.ndarray,
    y_train: np.ndarray,
    weights: np.ndarray,
    x_eval: np.ndarray,
) -> tuple[np.ndarray, Dict[str, Any]]:
    logistic, logistic_artifacts = fit_plain_probabilities(
        x_train, y_train, weights, x_eval
    )
    scaler, mlp = _fit_mlp(x_train, y_train)
    mlp_probabilities = _aligned_probabilities(
        mlp, mlp.predict_proba(scaler.transform(x_eval))
    )
    return 0.5 * (logistic + mlp_probabilities), {
        **logistic_artifacts,
        "mlp_scaler": scaler,
        "mlp": mlp,
...
def crossfit_model_probabilities(
    records: pd.DataFrame, features: np.ndarray
) -> tuple[Dict[str, np.ndarray], list[Dict[str, Any]]]:
    outputs = {
        name: np.full((len(records), N_CLASSES), np.nan, dtype=np.float64)
        for name in ("plain_logistic", "isotonic_logistic", "logistic_mlp_average")
    }
    ...
    for fold in OUTER_FOLDS:
        ...
        for held_inner in (1, 2, 3):
            ...
            train = records.iloc[train_positions]
            eval_rows = records.iloc[eval_positions]
            if set(train["participant"]) & set(eval_rows["participant"]):
            ...
            plain, plain_artifacts = fit_plain_probabilities(
                x_train, y_train, weights, x_eval
            )
            isotonic, iso_artifacts = fit_isotonic_probabilities(
                features, records, train_positions, eval_positions
            )
            ensemble, ensemble_artifacts = fit_ensemble_probabilities(
                x_train, y_train, weights, x_eval
            )
            outputs["plain_logistic"][eval_positions] = plain
            outputs["isotonic_logistic"][eval_positions] = isotonic
            outputs["logistic_mlp_average"][eval_positions] = ensemble
            audit.append(
                {
                    "outer_fold": fold,
                    ...
                    "eval_cases": eval_rows["case_id"].nunique(),
                    "participant_overlap": 0,
                    "plain_logistic_max_n_iter": plain_artifacts["max_n_iter"],
                    "isotonic_logistic_max_n_iter": iso_artifacts["max_n_iter"],
                    "mlp_n_iter": ensemble_artifacts["mlp_n_iter"],
                    "all_convergence_assertions_passed": True,
                }
            )
    for name, values in outputs.items():
        if not np.isfinite(values).all():
            raise RuntimeError(f"Missing/non-finite cross-fitted probabilities: {name}")
        if not np.allclose(values.sum(axis=1), 1.0, atol=1e-8):
            raise RuntimeError(f"Probability rows do not sum to one: {name}")
    return outputs, audit


def fit_final_model_probabilities(
    model_variant: str,
    records: pd.DataFrame,
    features: np.ndarray,
    train_positions: np.ndarray,
    ...
    weights = train["length"].to_numpy(dtype=float)
    x_eval = features[eval_positions]
    if model_variant == "plain_logistic":
        return fit_plain_probabilities(x_train, y_train, weights, x_eval)
    if model_variant == "isotonic_logistic":
        return fit_isotonic_probabilities(
            features, records, train_positions, eval_positions
        )
    if model_variant == "logistic_mlp_average":
        return fit_ensemble_probabilities(x_train, y_train, weights, x_eval)
...
def apply_configuration(
    ...
    *,
    ...
    probability_lookup = {
        int(segment_id): probabilities[position]
    ...
    for chosen in selected.itertuples(index=False):
        ...
        vector = probability_lookup[int(chosen.segment_id)]
        proposal = int(np.argmax(vector))
        ...
        eligible = confidence >= threshold and proposal != incumbent
        reason = "eligible"
