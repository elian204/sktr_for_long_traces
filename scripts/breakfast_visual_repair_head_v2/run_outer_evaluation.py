#!/usr/bin/env python3
"""Execute the approved, single-shot outer evaluation for repair-head v2."""
...
def _write_findings(path: Path, metrics: pd.DataFrame, decision: Mapping[str, Any]) -> None:
    ...
    lines = [
        "# Repair-head v2 single-shot outer findings",
        "",
...
def execute(study_dir: Path, verify_workers: int) -> None:
    ...
    for fold in OUTER_FOLDS:
        ...
        probabilities, artifacts = fit_final_model_probabilities(
            fold_config["model_variant"],
            records,
            features,
            train_positions,
            eval_positions,
        ...
        model_audit.append(
            {
                "outer_fold": fold,
                "model_variant": fold_config["model_variant"],
    ...
    for fold in OUTER_FOLDS:
        ...
        predictions, ledger, videos = apply_configuration(
            ...
            rule_parameter=float(fold_config["rule_parameter"]),
        )
